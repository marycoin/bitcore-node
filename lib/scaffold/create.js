...
var BASE_PACKAGE = {
  description: 'A full Marycoin node build with Bitcore',
...
/**
 * Will create a directory and marycoin.conf file for Marycoin.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createmarycoindirectory(datadir, done) {
 ...
 * @param {String} options.datadir - The marycoin database directory
 ...
 * marycoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the bitcore node configuration directory
 * @param {String} options.datadir - The path to the marycoin datadir
...
function create(options, done) {
  ...
  async.series([
    ...
    function(next) {
      // Setup the marycoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createmarycoindirectory(absDataDir, next);
      } else {
        next();
      }
    },
